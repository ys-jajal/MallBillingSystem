        ...
        Select Case CurrentTransmissionType
            Case TransmissionType.Text
                ...
                Try
                    ...
                    'Dim ds As DataSet
                    'ds = obj.gettablevalue(tmp(i))
                    'frmDisplay.DataGridView1.DataSource = ds.Tables("product")
                Catch
                End Try
                ...
                '        If _msg.Contains("0000001") Then

                '            tmp = "0000001"
                '            ' MsgBox(tmp)
                '            ds = obj.gettablevalue(tmp)
                '            'Dim lst As New ListViewItem(tmp)
                '            'lst.SubItems.Add(tmp)
                '            'frmBilling.ListView1.Items.Add(lst)
                '            'frmDisplay.DataGridView1.DataSource = ds.Tables("Product")
                '            frmBilling.DataGridView1.DataSource = ds.Tables("Product")
                '            'frmBilling.dgitem.DataSource = ds.Tables("Product")



                '        End If
                '        If _msg.Contains("0000002") Then
                '            tmp = "0000002"

                '            ds.Merge(obj.gettablevalue(tmp))
                '            frmBilling.DataGridView1.DataSource = ds.Tables("Product")
                '            'frmBilling.dgitem.DataSource = ds.Tables("Product")
                '            'frmDisplay.DataGridView1.Rows.Add(ds.Tables("product"))
                '            'frmDisplay.DataGridView1.DataSource = ds.Tables("product")

                '        End If
                '        If _msg.Contains("0000003") Then
                '            tmp = "0000003"
                '            ds.Merge(obj.gettablevalue(tmp))
                '            frmBilling.DataGridView1.DataSource = ds.Tables("Product")
                '            'frmBilling.dgitem.DataSource = ds.Tables("Product")
...
#End Region
    ...
    Function checkcode(ByVal tmp As String)
        ...
        con.Open()
        Dim Sql As String = "select * from Product where Prod_No='" & tmp & "'"
        ...
        If r1.Read Then
            'MsgBox(r1("PName"))
            Dim temp As Integer
            temp = frmBilling.ListView1.Items.Count()
            ...
            lst.SubItems.Add("1")
            lst.SubItems.Add(r1("PPrice"))
            lst.SubItems.Add(r1("Prod_No"))
            ' lst.SubItems.Add(txtpno.Text)
            frmBilling.ListView1.Items.Add(lst)
            frmBilling.txtsubtot.Text = frmBilling.subtot()
            'TextBox1.Text = r("uname")
            'TextBox2.Text = r("lastname")
            'TextBox4.Text = r("rollno")
            'TextBox5.Text = r("phoneno")
            'ComboBox1.Text = r("category")
        End If
